...
max_x = 1000
min_y = 0
max_y = max(pos[1] for path in rock_paths for pos in path) +2
...
def get_stone_end_a_pos(drop_a_pos):
    ...
    if y_fundament_base is None:
        y_fundament_base = max_y-min_y
    fundament_base = (drop_a_pos[0], y_fundament_base)
    if y_fundament_base == max_y-min_y:
        fundament = ['#']*3
    else:
...
while True:
    ...
    num_sands += 1
    if end_pos == sand_a_source:
        break
    cave[end_pos[1]][end_pos[0]] = 'o'

print(f'snd part sol: {num_sands=}')